'''
tracks_mots_g8_env_val /home/ajin/Data/kitti/images demo_0114 datasets/mots_tools/mots_eval/val.seqmap
tracks_mots_g22_emb_val /home/ajin/Data/kitti/images demo_0305 datasets/mots_tools/mots_eval/val.seqmap

/data/xuzhenbo/PointTrack/testset_result_20200213 /data/xuzhenbo/kitti/testing/images /data/xuzhenbo/PointTrack/testset_video_0313 datasets/mots_tools/mots_eval/test.seqmap
/data/xuzhenbo/PointTrack/tracks_mot_pointtrack_testset /data/xuzhenbo/MOT17/images /data/xuzhenbo/PointTrack/MOT_video_0313 datasets/mots_tools/mots_eval/test_MOTS20.seqmap
'''
...
from datasets.mots_tools.mots_common.io import load_sequences, load_seqmap
...
def visualize_sequences(seq_id, tracks, max_frames_seq, img_folder, output_folder, draw_boxes=False, create_video=True):
        ...
        if t in tracks:
            for obj in tracks[t]:
                ...
                if obj.class_id == 1 or obj.class_id == 2:  # Don't show boxes or ids for ignore regions
                    ...
                    # ax.annotate(category_name, (x + 0.5 * w, y-5), color=color, weight='bold', fontsize=10, ha='center', va='center', alpha=1.0)
    ...
    if create_video:
        ...
        currentF = os.getcwd()
        shutil.copyfile(os.path.join(currentF, "output.mp4"), os.path.join(currentF.rsplit('/', 1)[0], seq_id + '.mp4'))
