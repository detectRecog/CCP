"""
for person, crop should be upsample for data distribution
1.small instance in KINS is dropped.
2.bigger crop size
Author: Zhenbo Xu
...
def process(tup):
    ...
    for j, id in enumerate(ids):
        ...
        if kins_image and instw < 50:
            if instw >= 30:
                # w<60 KINS instances should be upsampled by 60%, 1/1.6=0.625
        ...
        occlude_ratio = occlude_ratio_ if kins_image else max(0.5, occlude_ratio_)
...
if __name__ == '__main__':
    ...
    print(dst_kins, ', class_id: ', class_id, ', CROP_SIZE: ', CROP_SIZE_, ', max_occupy: ', max_occupy, ', dratio: ', dratio)
    ...
    # mots_persons = load_pickle(os.path.join(kittiRoot, 'mots_person_train5.pkl'))
    mots_persons = load_pickle(os.path.join(kittiRoot, 'mots_train_set.pkl'))
